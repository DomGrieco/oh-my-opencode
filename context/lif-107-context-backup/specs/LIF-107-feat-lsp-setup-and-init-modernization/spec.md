...
The oh-my-opencode plugin supports 21 built-in LSP servers (TypeScript, Python, Rust, Go, C/C++, Java, Ruby, Swift, Dart, and more) with 11 LSP tools available to AI agents. Project initialization tooling exists in `.opencode/tool/init-project/` with tech detection, config generation, and AGENTS.md templating capabilities. The plugin works with sensible defaults once configured.
...
**Acceptance Criteria:**
- [ ] Index page lists all 21 supported language servers
...
Dedicated documentation pages for each of the 21 supported language servers. Each card must include: installation command, supported file extensions, verification method, 3 most common issues with solutions, and advanced configuration options.
...
- LSP setup documentation for all 21 built-in servers
...
- Adding new LSP servers beyond the existing 21
...
5. The 21 built-in LSP servers remain stable (install commands won't change frequently)
...
**Context**: Users typically care about 1-3 languages, not all 21.
...
1. **LSP Server Priority Display**: How should we indicate recommended servers when multiple options exist for a language (e.g., pyright vs basedpyright)? Should it be configurable per-project?

2. **Init Command Location**: Should the command be `/init-project` (current convention) or `/init` (shorter)? Does the shorter form conflict with any existing OpenCode functionality?

3. **Telemetry for Success Metrics**: How will we measure init completion rate and LSP setup success without adding intrusive telemetry? Should this be opt-in?
